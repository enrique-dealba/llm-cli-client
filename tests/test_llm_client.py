...
def test_process_default_response_with_text_field():
    """Test process_default_response func with a text field."""
    input_data = {"text": "This is some text generated by the AI that we want to output"}
    assert process_default_response(input_data) == input_data["text"]
...
def test_generate_command_default():
    ...
    mock_response = {
        "text": "This is some generated text from the AI."
    }
    ...
    assert result.exit_code == 0
    assert mock_response["text"] in result.output
...
def test_process_skill_command_default():
    ...
    mock_response = {
        "text": "This is the processed skill output."
    }
    with patch("requests.post") as mock_post:
        mock_post.return_value.json.return_value = mock_response
        mock_post.return_value.raise_for_status.return_value = None
